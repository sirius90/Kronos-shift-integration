...
namespace Microsoft.Teams.Shifts.Integration.API.Controllers
{
    ...
    public class SyncKronosToShiftsController : ControllerBase
    {
        ...
        /// <param name="countryCode"> Country code set by the LogicApp QueryString.</param>
        /// <returns>Returns Ok if request is successfully queued.</returns>
        [Authorize(Policy = "AppID")]
        [HttpGet]
        [Route("StartSync/{isRequestFromLogicApp}/{countryCode}")]
        public ActionResult SyncKronosToShifts(string isRequestFromLogicApp, string countryCode)
        {
            ...
            // Change the static string to the queryString countryCode
            ApiConstants.SoapEnvOpen = ApiConstants.SoapEnvOpen.Replace("USA", countryCode, StringComparison.CurrentCultureIgnoreCase);
